...
describe('CtxInspectTool', () => {
  ...
  test('reports context with messages', async () => {
    const mockCtx: ToolUseContext = {
      ...
      abortSignal: new AbortController().signal,
      messages: [
        { role: 'user', type: 'user', message: { role: 'user', content: 'Hello', uuid: '1', timestamp: '', sessionId: 's1' } },
        { role: 'assistant', type: 'assistant', message: { role: 'assistant', content: [{ type: 'text', text: 'Hi there!' }], model: 'test', stop_reason: null, stop_sequence: null, usage: { input_tokens: 10, output_tokens: 5 } }, uuid: '2', timestamp: '', sessionId: 's1' },
      ],
      isInteractive: false,
    }
  ...
  test('returns metadata with token info', async () => {
    const mockCtx: ToolUseContext = {
      ...
      abortSignal: new AbortController().signal,
      messages: [
        { role: 'user', type: 'user', message: { role: 'user', content: 'Test', uuid: '1', timestamp: '', sessionId: 's1' } },
      ],
      isInteractive: false,
    }
    const result = await CtxInspectTool.execute(mockCtx, {})
