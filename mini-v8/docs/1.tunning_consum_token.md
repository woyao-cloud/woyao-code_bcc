...
# P2
  ...
  history。
   ...
   已做完。context.ts 现在变成预算化的上下文组装器：先从 messagesForAPI 抽 query，再按 maxContextTokens 选择性注入
  CLAUDE.md、skills、memories、agents、teams；可选块默认相关才进 prompt，超预算就直接跳过。memoryStore.ts 和
  agentRegistry.ts 也改成了 query-aware top-k，不再每轮全量拼大块。

  测试和类型检查都过了：

  - bun test src/services/messages/__tests__/apiProjection.test.ts src/services/memory/__tests__/memoryStore.test.ts
    src/__tests__/context.test.ts src/services/memory/__tests__/sessionMemory.test.ts src/__tests__/autoCompact.test.ts
  - bun run typecheck

  关键位置：[context.ts](D:\claude-code-project\claude-code-best\woyao-code_bcc\mini-v8\src\context.ts:39),
  [memoryStore.ts](D:\claude-code-project\claude-code-best\woyao-code_bcc\mini-
  v8\src\services\memory\memoryStore.ts:271), [agentRegistry.ts](D:\claude-code-project\claude-code-best\woyao-
  code_bcc\mini-v8\src\agents\agentRegistry.ts:128)
