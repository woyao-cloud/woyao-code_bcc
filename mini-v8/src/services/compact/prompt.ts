/**
 * LLM compaction prompt templates.
 * Ported from the main codebase — pure string templates with no dependencies.
 *
 * These prompts are used to generate structured conversation summaries
 * via an API call. The model produces an <analysis> scratchpad followed
 * by a <summary> with 9 structured sections.
 */

type PartialCompactDirection = 'from' | 'up_to'

// Aggressive no-tools preamble. With maxTurns: 1, a denied tool call means
// no text output. Putting this FIRST and making it explicit about rejection
